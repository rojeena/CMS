...
(function ($) {
    ...
    MYAPP.carouselsAuto = {
        ...
        eventsCarousel: true, //** Lates Work carousel in main page
    ...
    jQuery(document).ready(function () {
        ...
        { //** NOT on mobile
            $('.events-container').addClass('notMobile');
        }
        ...
        imagesLoaded($('.eventsCarousel'), function (instance) {
            $('.eventsCarousel').startTransparencyCarousel(5, { auto: MYAPP.carouselsAuto['eventsCarousel'] });
        });

      //  $(document).on('tap', '.eventsCarousel li', function (e) {
//            e.preventDefault();
...
//            var currentPortfolioItemContent = $('.eventsCarousel [data-id="' + portfolioId + '"]');
...
//        });
        ...
        $('#mainMenu li a').click(function () {
            ...
            $(el).animatescroll();
            setTimeout(function () { $('body').removeClass('menuOpened'); }, 100);
            return false;
        });
        ...
        $(document).on('click', '#btn-send', function (e) {
            ...
            if(form.valid()) {
                ...
                $.ajax({
                    ...
                    dataType: 'JSON',
                    beforeSend: function() {
                        $('#contactResponse').html("<p class='alert alert-info'>Processing request</p>");
                    },
                    success: function (data) {
                        $('#contactResponse').html("<p class='alert alert-info'>" + data.msg + "</p>");
        ...
        $('.newsletter-form').on('submit', function (e) {
            ...
            //everything looks good! proceed...
            if (proceed) {
                $('.newsletter-form .contactBusy').css({'display':'block'});
                //data to be sent to server
                ...
                $.post($(this).attr('action'), post_data, function (data) {
                    console.log(data);
                    $('.newsletter-form .contactBusy').css({ 'display': 'none' });
                    $('#subscribeResponse').html(data);
                    $('#subscribeResponse').alert();
                    //reset values in all input fields
                    $('#contactForm input').val('');

                }).fail(function (err) {  //load any error data
                    $('.newsletter-form .contactBusy').css({ 'display': 'none' });
                    $('#subscribeResponse').html('<div class="alert alert-danger alert-dismissable fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><strong>Error! </strong>' + err.statusText + '</div>');
                    $('#subscribeResponse').alert();
                });
            }
        });
    ...
    $.fn.startTransparencyCarousel.defaults = {
            ...
            prev: { button: function () { return $(this).closest('.events-container').find('.Prev'); }, key: "left" },
            next: { button: function () { return $(this).closest('.events-container').find('.Next'); }, key: "right" }
        };
    ...
    var PortfolioItem = function (listItem) {
        ...
        if (MYAPP.carouselsAuto['eventsCarousel'])
            $('.eventsCarousel').trigger('stop', true); //** stop the carousel auto scroll
                ...
                function () {
                    ...
                    $(".untitledModal").animate({ opacity: 0 }, 200,
                        function () {
                            ...
                            if (MYAPP.carouselsAuto['eventsCarousel'])
                                $('.eventsCarousel').trigger('play', true);
